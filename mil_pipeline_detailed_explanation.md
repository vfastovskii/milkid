...
| Role | Class | File |
|---|---|---|
| Contextualized embedder | `ContextualizedMLPEmbedder` (wraps `MLPEmbedderV3` + `BagSelfAttentionBlock`) | `ppl/models/components/embedders/contextualized_mlp_embedder.py` |
...
| Predictor | `MLPPredictorV3` (uses `_ResFFNBlock`) | `ppl/models/components/predictors/mlp_predictor_v3.py` |
...
- **Predictor head (`MLPPredictorV3`).** A residual SwiGLU MLP maps `bag_repr ∈ ℝ^D` to the scalar `ŷ`.
- **Training.** The supervised objective is mean-squared error between `ŷ` and the experimental activity, plus a small cluster-structure regularizer emitted by the aggregator. Optimization is AdamW with per-component learning rates, decoupled weight decay (biases/norms excluded), and a `ReduceLROnPlateau` scheduler monitoring `val_rmse`. Training is orchestrated with PyTorch Lightning and includes checkpointing on best `val_rmse`, an "attention-refinement" post-overfit phase, and an optional overfit-gap early stop.
...
flowchart TD
    ...
    X --> LEMB["MLPEmbedderV3 (shared, per-conformer)<br/>in_proj F→H · L=3 residual SwiGLU FFN blocks<br/>pre-LN · LayerScale γ · DropPath · fc_out→D"]
    ...
    SA --> Z["z: [B, N, D]  (D = 512)<br/>contextualized conformer embeddings"]
    ...
    BR --> PRED["MLPPredictorV3<br/>input-LN · 2 residual SwiGLU blocks · head_dropout · Linear(H→1)"]
...
`B` = bags (molecules) in a batch (`batch_size: 16`). `N` = `N_max`, the padded conformer count for the batch (variable; the raw dataset uses up to ~200 conformers per molecule after pruning). `F` = input descriptor width (injected at runtime, see §4.2). `D` = embedding width = `512`. `C` = number of conformer clusters in a bag (≤ `cluster_max_clusters: 6`). `K` = number of query tokens = `1`. `H_heads` = attention heads.
...
| Local embed | `h = MLPEmbedderV3(x)` | `[B, N, D]` | Per-conformer latent (pre-context) | Padded rows zeroed by mask |
...
`bag_repr` is `[B, D]` with `D = 512`; the predictor's `output_dim = 1` yields `[B, 1]`, which `MILCore` flattens to `[B]` (or a 0-d scalar when `B = 1`).
...
- **`F`** — the per-conformer descriptor width. This is the concatenated 3D-fingerprint feature vector for a conformer.
  > **`F` is not specified statically in the model code.** It is injected at build time as `ModelBuilderConfig.input_dim`, computed from the processed descriptor count of the loaded dataset (`ModelFactory.build_model(cfg, input_dim, task)`; `MLPEmbedderV3(input_dim=…)`). The shipped dataset is `bace809_3d_fp_5fp_200c_05prune_6kcal_no_Hs_june_concated_splited.csv` (a 5-fingerprint 3D descriptor set); to read the exact `F`, inspect the number of descriptor columns after preprocessing.
- **`D`** — the embedding width, `embed_dim: 512` in the shipped config (the config comment notes the paper used `256`).

### 4.3 Layer-by-layer architecture

**Local embedder — `MLPEmbedderV3`** (constructed inside `ContextualizedMLPEmbedder.__init__`):

```text
input_dim = F, hidden_dim = D = 512, num_layers = 3, expansion = 2.0,
dropout = 0.2, output_dim = 512, final_norm = "none", block_norm = "pre",
gated = True (SwiGLU), stochastic_depth = 0.03, zero_init_last_block = True
```

Inner FFN width per block: `inner = ceil(expansion·hidden_dim / 64)·64 = ceil(1024/64)·64 = 1024`.
...
| 0 | `in_proj` = `Linear(F, 512)` | `[·, F]` | `[·, 512]` | — | — | — | Project descriptor to hidden width |
| 1 | Residual block 1: preLN → `ff1: Linear(512,2·1024)` → SwiGLU → drop → `ff2: Linear(1024,512)` → `·γ` → DropPath → `+x` | `[·,512]` | `[·,512]` | SiLU (gate) | LayerNorm (pre) | 0.2 | Nonlinear feature mixing |
| 2 | Residual block 2 (same) | `[·,512]` | `[·,512]` | SiLU | LayerNorm (pre) | 0.2 | " |
| 3 | Residual block 3 (same; `ff2` zero-init) | `[·,512]` | `[·,512]` | SiLU | LayerNorm (pre) | 0.2 | Near-identity start (LayerScale + zero-init) |
| 4 | `fc_out` — **absent** here (`output_dim == hidden_dim`, so `fc_out = None`) | `[·,512]` | `[·,512]` | — | — | — | Identity output head |

Per-block details from `MLPEmbedderV3`:
...
```text
dim = 512, num_heads = 2, mlp_ratio (attn_mlp_ratio) = 1.0,
attn_dropout = 0.0, dropout (attn_ffn_dropout) = 0.15,
drop_path (attn_stochastic_depth) = 0.02, residual_scale_init = 0.005
```

Inner FFN width: `ceil(512·1.0/64)·64 = 512`.

| # | Operation | Input | Output | Activation | Norm | Dropout | Purpose |
|---:|---|---|---|---|---|---|---|
| 1 | `norm1` = LayerNorm | `[B,N,512]` | `[B,N,512]` | — | LayerNorm (pre) | — | Pre-norm for attention |
| 2 | `attn` = `nn.MultiheadAttention` (2 heads, `batch_first`) with `key_padding_mask = ~mask` | `[B,N,512]` | `[B,N,512]` | softmax | — | 0.0 (attn) | Conformer↔conformer mixing |
| 3 | Residual: `x + DropPath(γ_attn · attn_out)` | `[B,N,512]` | `[B,N,512]` | — | — | DropPath 0.02 | LayerScale-gated residual |
| 4 | `norm2` = LayerNorm | `[B,N,512]` | `[B,N,512]` | — | LayerNorm (pre) | — | Pre-norm for FFN |
| 5 | FFN: `ff1: Linear(512,2·512)` → SwiGLU → dropout → `ff2: Linear(512,512)` | `[B,N,512]` | `[B,N,512]` | SiLU (gate) | — | 0.15 | Position-wise nonlinearity |
| 6 | Residual: `x + DropPath(γ_ffn · ffn_out)` | `[B,N,512]` | `[B,N,512]` | — | — | DropPath 0.02 | LayerScale-gated residual |
| 7 | Re-mask: `x · mask` (zero padded rows) | `[B,N,512]` | `[B,N,512]` | — | — | — | Keep padding at zero |

> The non-gated activation choice inside `MLPEmbedderV3` is `activation="gelu"` by default, **but** `gated=True` is set, so the effective activation on the gated branch is always **SiLU** (SwiGLU). GELU is never used on this path.

### 4.4 Embedder computation scheme

```mermaid
flowchart TD
    X["x: [B, N, F]"] --> IP["in_proj: Linear F→512 (shared over B·N)"]
...
```text
h_{i,j} = E_θ(x_{i,j}),           h_{i,j} ∈ ℝ^D,   E_θ = MLPEmbedderV3
```
...
- **Multi-head attention** uses `num_heads = 4`, so `head_dim = 512/4 = 128`. Heads are the standard `nn.MultiheadAttention` concatenation + output projection; the block adds residual connections, pre-LayerNorm, LayerScale γ, and a SwiGLU FFN (inner width `ceil(512·1.0/64)·64 = 512`).
...
`MLPPredictorV3` maps the pooled molecular representation `bag_repr ∈ ℝ^D` to the scalar activity. It is a residual SwiGLU MLP with an input LayerNorm to stabilize the (possibly variable-scale) `bag_repr`, and no output activation (raw regression score).

### 6.2 Layer-by-layer table

Config: `input_dim = D = 512` (equals `bag_repr` width), `hidden_dim = 256`, `num_layers = 2`, `expansion = 2.0`, `activation = "silu"`, `use_glu = True`, `dropout = 0.2`, `stochastic_depth = 0.08`, `head_dropout = 0.2`, `output_dim = 1`, `input_layernorm = True`, `final_layernorm = False`, `res_scale_init = 0.1`, `inner_multiple = 64`. Inner width `= ceil(256·2.0/64)·64 = 512`.

| Layer | Operation | Input | Output | Activation | Dropout | Purpose |
|---:|---|---|---|---|---|---|
| 0 | `input_norm` = LayerNorm(512) | `[B, 512]` | `[B, 512]` | — | — | Normalize bag_repr scale |
| 1 | `_ResFFNBlock` #1: preLN → `fc1: Linear(512, 2·512)` → SwiGLU → dropout → `fc2: Linear(512, 256)`; residual via `proj: Linear(512,256)`; `·tanh(res_scale)`; DropPath | `[B, 512]` | `[B, 256]` | SiLU (gate) | 0.2 | Nonlinear projection to hidden |
...
| Embedder LR | `2.5479848e-5` | Local `MLPEmbedderV3` group |
...
| Predictor LR | `9.548040e-6` | `MLPPredictorV3` group |
...
Notes:
- The embedder is split into two groups: the **local embedder** (`MLPEmbedderV3`) gets `embedder_lr`, while the `context_blocks` self-attention gets `self_attention_lr`. Each group is further split into `.decay` (weights, `weight_decay` applied) and `.no_decay` (biases + LayerNorm params + LayerScale γ, `weight_decay = 0`).
...
| monitor | `val_rmse` (regression, when val data present) | Falls back to `train_loss_epoch` if no val |
| patience (`lr_patience`) | `15` | Epochs without improvement before decay |
| factor | `0.01` | LR is multiplied by `0.01` on plateau (aggressive) |
| min_lr | `1e-6` | LR floor |
| interval / frequency | `epoch` / `1` | Steps once per epoch |

Other schedulers are implemented but inactive: `cosine` (`CosineAnnealingLR`, `T_max = lr_t_max = 50`, `eta_min = 5e-6`), `one_cycle` (`OneCycleLR`), `step` (`StepLR`), and `none` (constant LR).

---

## 8. Trainer and training loop

Training is driven by PyTorch Lightning through `MILModelLightningWrapper` (mixes in `TrainingMethods`, `OptimizationMethods`, `MemoryManagement`, `ParameterManagement`). Data flows from `MILDataset` → `collate_mil` → `DataLoader`.
...
- `batch_size = 16`; training batches can be **series-balanced** (`balance_train_batches_by_series: true`) via a custom sampler.
...
for epoch in range(max_epochs):
    ...
    scheduler.step(val["rmse"])                            # ReduceLROnPlateau on val_rmse
    maybe_trigger_attention_refinement(val)                # post-overfit phase
    maybe_overfit_gap_stop(val)                            # optional early stop
    checkpoint_if_best(val_rmse, min_epoch=30)
```
...
- `scheduler.step` fires per epoch on `val_rmse`.
...
- `EarlyStopping` (monitor `val_rmse`, `mode="min"`, `patience = lr_patience = 15`) is built **only when `attention_refinement_enabled` is False**. In the shipped config `attention_refinement_enabled: true`, so Lightning's early stopping is disabled and the attention-refinement controller owns the post-overfit phase.
...
- **Attention refinement** (`_maybe_update_attention_refinement_schedule`): when val stops improving and the val−train gap crosses `attention_refinement_gap_threshold = 0.15` (abs) or `rel_gap ≥ 0.15` for `patience = 1` epoch, it (a) **reduces embedder/predictor LRs** by `attention_refinement_lr_factor = 0.01`, and (b) **ramps the active-prototype query weight** `0 → 1` over `attention_refinement_query_epochs = 25` epochs, then stops training (`stop_after_query_epochs: true`). This is a curriculum that first fits the predictor, then shifts emphasis to the prototype-informed key-instance query.

**Checkpointing.** `ModelCheckpoint`/`MinEpochModelCheckpoint` monitors `val_rmse` (`mode="min"`, `save_top_k = 1`), gated to fire no earlier than `checkpoint_min_epoch = 30` and, when configured, only after attention refinement (`checkpoint_after_attention_refinement: true`, `checkpoint_min_query_epochs = 20`). A `LearningRateMonitor` and (optionally) `TQDMProgressBar` + `ModelSummary(max_depth=2)` are also attached.
...
| RMSE | `MeanSquaredError(squared=False)` | Root mean squared error; **the checkpoint/scheduler monitor** |
...
```mermaid
flowchart LR
    A["x [B, N, F]"] --> B["in_proj → [B, N, 512]"]
    B --> C["3× residual SwiGLU → [B, N, 512]"]
    C --> D["self-attn (heads=2) → z [B, N, 512]"]
    D --> E["cluster tokens [B, C, 512]"]
...
```mermaid
flowchart TD
    BR["bag_repr [B, 512]"] --> IN["LayerNorm(512)"]
    IN --> R1["_ResFFNBlock #1: SwiGLU 512→512→256 (+proj residual)"]
...
flowchart TD
    ...
    VM --> SCH["ReduceLROnPlateau.step(val_rmse)"]
    SCH --> AR["attention-refinement / overfit-gap controllers"]
    AR --> CK["checkpoint best val_rmse (min_epoch=30)"]
    CK -->|not stopped| L
```
...
|---|---|---|---|---|---|
| `ContextualizedMLPEmbedder` → `MLPEmbedderV3` | `in_proj`, 3× SwiGLU FFN (`ff1/ff2`), LayerScale γ, LayerNorms | `x [B,N,F]` | `h [B,N,512]` | Yes | Per-conformer latent encoding (weight-shared) |
| `ContextualizedMLPEmbedder` → `BagSelfAttentionBlock` ×1 | `MultiheadAttention` (heads=2), SwiGLU FFN, γ_attn/γ_ffn, LayerNorms | `h [B,N,512]` | `z [B,N,512]` | Yes | Cross-conformer contextualization |
| `ClusterHierarchicalAttentionAggregator` | learned `cluster_cls` query, `cluster_block`/`instance_block` (heads=4, Q/K/V + FFN + γ), `refine_query_proj`, `external_query_proj`+gate, `log_tau` | `z [B,N,512]`, mask, `cluster_ids` | `bag_repr [B,512]`, `alpha [B,N]`, `reg_loss` | Yes | Hierarchical key-instance pooling |
| `MLPPredictorV3` | input LN, 2× `_ResFFNBlock` (SwiGLU + `res_scale`), `fc_out` | `bag_repr [B,512]` | `ŷ [B,1]→[B]` | Yes | Scalar activity prediction |
...
| `ActivePrototypeQuery` | `fallback_query`, `query_proj` (LN+Linear+Tanh) | `z`, bank | external query `[B,1,512]` | Yes | Build prototype-informed CLS query |
...
- **Overfitting risk with small data.** The dataset here is on the order of a few hundred molecules (BACE-809, `batch_size = 16`, ~37 train batches per the config comment). The model stacks several transformer-style attention modules; the config leans heavily on regularization (dropout 0.2 in embedder/predictor, stochastic depth, LayerScale near-identity init, weight decay ~3e-3, aggressive plateau LR decay) and on **train/val-gap-triggered controllers** (attention refinement, optional overfit-gap stop). Monitor the RMSE/MAE train–val gap explicitly.
...
Each molecule is encoded as a padded bag of `N` conformer descriptor vectors `x ∈ ℝ^{B×N×F}`; a weight-shared, SwiGLU-gated residual MLP (`MLPEmbedderV3`) maps every conformer independently to `ℝ^{512}`, and a single masked multi-head self-attention block (`BagSelfAttentionBlock`) contextualizes conformers within each bag to yield `z ∈ ℝ^{B×N×512}`. A hierarchical query-attention aggregator (`ClusterHierarchicalAttentionAggregator`) mean-pools `z` into precomputed conformer clusters, lets a learned CLS query cross-attend over cluster tokens to score clusters, refines that query and cross-attends over individual conformers, and blends the coarse (cluster-uniform) and fine (instance) attentions into a masked, normalized per-conformer distribution `alpha ∈ ℝ^{B×N}`, whose weighted sum over conformer features gives the permutation-invariant bag representation `bag_repr ∈ ℝ^{B×512}`; a residual SwiGLU MLP head (`MLPPredictorV3`) maps `bag_repr` to a scalar activity `ŷ`. The model is trained with `MILModelLightningWrapper` to minimize `MSE(ŷ, y)` plus a small cluster-compactness/separation regularizer, using AdamW with per-component learning rates and decoupled weight decay, a `ReduceLROnPlateau(val_rmse)` schedule, best-`val_rmse` checkpointing, and an attention-refinement curriculum that, upon an emerging train–val gap, lowers the embedder/predictor learning rates and ramps in an active-conformer-prototype query. Molecule-level accuracy is measured by MAE/RMSE/R²/Pearson/Spearman, while the exposed attention and mask-ablation impact scores provide an interpretable, externally-validatable signal for bioactive key-conformer identification.
